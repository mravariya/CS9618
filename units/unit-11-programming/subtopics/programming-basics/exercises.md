...
1. **Define** one key term from `Programming Basics` using syllabus-quality wording.
2. **State** one real-world application and **link** it to a technical idea from this subtopic.
3. **Give** one advantage and one limitation of a method discussed in `Programming Basics`.
...
3. **Justify** a recommendation that balances **performance**, **security**, and **usability** (choose factors relevant to `Programming Basics`).

## Scenario-based

You are advising a small organisation that must keep records consistent and secure.

- **Identify** two **risks** relevant to `Programming Basics`.  
