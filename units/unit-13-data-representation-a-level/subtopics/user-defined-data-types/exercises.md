# Exercises — User-defined Data Types

_Original exam-style questions only. Do not copy Cambridge past papers verbatim._

## Short answer

1. **Define** one key term from `User-defined Data Types` using syllabus-quality wording.
2. **State** one real-world application and **link** it to a technical idea from this subtopic.
3. **Give** one advantage and one limitation of a method discussed in `User-defined Data Types`.
...
3. **Justify** a recommendation that balances **performance**, **security**, and **usability** (choose factors relevant to `User-defined Data Types`).

## Scenario-based

You are advising a small organisation that must keep records consistent and secure.

- **Identify** two **risks** relevant to `User-defined Data Types`.  
