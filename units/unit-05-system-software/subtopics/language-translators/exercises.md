# Exercises — Language Translators

_Original exam-style questions only. Do not copy Cambridge past papers verbatim._

## Short answer

1. **Define** one key term from `Language Translators` using syllabus-quality wording.
2. **State** one real-world application and **link** it to a technical idea from this subtopic.
3. **Give** one advantage and one limitation of a method discussed in `Language Translators`.

## Structured question

A school is updating its systems. A scenario will describe users, devices, and data.

1. **Describe** how a concept from **System Software** applies to the scenario.  
2. **Explain** why an alternative approach might be weaker **in this context**.  
3. **Justify** a recommendation that balances **performance**, **security**, and **usability** (choose factors relevant to `Language Translators`).

## Scenario-based

You are advising a small organisation that must keep records consistent and secure.

- **Identify** two **risks** relevant to `Language Translators`.  
